...
.appt-confirmed{ background: #f0fdf4; border-left: 4px solid #22c55e; padding: 10px 16px; border-radius: 8px; margin-bottom: 6px; }
...
def get_doctor_appointments(doctor_id: int) -> pd.DataFrame:
    con = sqlite3.connect(DB_PATH)
    df  = pd.read_sql_query("""
        SELECT a.*, p.full_name AS patient_name, p.phone, p.email
        FROM appointments a
...
def update_appointment_status(appointment_id: int, status: str, notes: str = "") -> None:
    _write(
        "UPDATE appointments SET status=?, doctor_notes=? WHERE appointment_id=?",
    ...
    # ── Book Appointment ──────────────────────────────────────────────────────
        ...
        if not linked_docs:
            ...
            if appts.empty:
                ...
                STATUS_ICON  = {"pending": "🟡", "confirmed": "🟢", "rejected": "🔴", "completed": "⚪"}
                STATUS_CLASS = {"pending": "appt-pending", "confirmed": "appt-confirmed",
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    STATUS_COLOR = {"pending": "🟡", "confirmed": "🟢", "rejected": "🔴", "completed": "⚪"}
            ...
            with st.expander(
                ...
                with ca:
                    if st.button("✅ Confirm", key=f"conf_{row['appointment_id']}"):
                        update_appointment_status(row["appointment_id"], "confirmed", doc_note)
                        st.rerun()
                with cb:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if not all_appts.empty:
        ...
        fig_a = px.pie(appt_counts, names="Status", values="Count", hole=0.4,
                       color="Status",
                       color_discrete_map={"pending": "#F59E0B", "confirmed": "#22C55E",
